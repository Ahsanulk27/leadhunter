...
const VERIFICATION_DIR = path.join(os.tmpdir(), "leadhunter_verification");
...
export class VerificationController {
  ...
  async verifySingleLead(req: Request, res: Response) {
    try {
      ...
      console.log(
        `🔍 VerificationController: Starting verification ${executionId} for ${name}`
      );
  ...
  async verifyBatchLeads(req: Request, res: Response) {
    try {
      ...
      for (const lead of leads) {
        if (
          ...
          return res.status(400).json({
            success: false,
            error:
              "Each lead must have id, name, email, phoneNumber, and address fields",
          });
        }
      }
      ...
      const executionId = generateExecutionId();
      console.log(
        `🔍 VerificationController: Starting batch verification ${executionId} for ${leads.length} leads`
      );
