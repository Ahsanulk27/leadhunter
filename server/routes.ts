...
function calculateDataQualityScore(businesses: BusinessData[]): number {
  ...
  for (const business of businesses) {
    ...
    if (business.contacts && business.contacts.length > 0) {
      ...
      const hasDecisionMakers = business.contacts.some(
        (c) => c.isDecisionMaker
      );
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Register bulk lead routes under /api/bulk-leads
  app.use("/api/bulk-leads", (req, res, next) => {
    // Simple CORS handling for bulk-leads routes
    res.header("Access-Control-Allow-Origin", "*");
    res.header("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE");
    res.header("Access-Control-Allow-Headers", "Content-Type");
    next();
  });
  const bulkLeadRouter = createBulkLeadRouter(googlePlacesService);
  app.use("/api/bulk-leads", bulkLeadRouter);
  ...
  app.post("/api/search", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        "📍 Searching for REAL business data using Google Places API..."
      );
      ...
      try {
        ...
        console.log(
          ...
          scrapedData ? "Data found" : "No data found"
        );
        ...
        if (!scrapedData) {
          ...
          if (!industry && !company && !location) {
            return res.status(404).json({
              error: "Missing search criteria",
              message:
                "Please provide either an industry, a company name, or a location to search for leads.",
            });
          } else {
            return res.status(404).json({
              error: "No real business data found for this search",
              message:
                "No real business information could be found after searching Google Maps, Yelp, and Yellow Pages. Try searching with different criteria, such as a more specific company name or location.",
            });
          }
        }
      } catch (apiError: any) {
          ...
          if (apiError.message.includes("REQUEST_DENIED")) {
            ...
            return res.status(429).json({
              error: {
                ...
                message:
                  "We've reached our daily search limit with Google Places API. Please try again tomorrow.",
              },
            });
          } else {
            return res.status(503).json({
  ...
  app.post("/api/export-to-sheets", async (req: Request, res: Response) => {
    try {
      console.log(
        "📊 API /export-to-sheets route called with request body:",
        req.body
      );
      ...
      const executionId = `export-${Date.now()}-${Math.floor(
        Math.random() * 1000
      )}`;
      ...
      const { googleSheetsService } = await import(
        "./api/google-sheets-service"
      );
      const { searchController } = await import(
      ...
      if (searchId) {
        const searchHistory = await storage.getSearchHistory();
        const searchEntry = searchHistory.find(
          (entry) => entry.id === parseInt(searchId)
        );
        ...
        console.log(
          `📊 [${executionId}] Found search in history: "${searchEntry.query}"`
        );
        ...
        if (searchEntry.query.includes(" in ")) {
          ...
          [industry, location] = searchEntry.query
            .split(" in ")
            .map((s) => s.trim());
        } else {
          // Assume it's a company name or just industry/location
          if (
            /[A-Z]/.test(searchEntry.query) ||
            /[&\-',.]/.test(searchEntry.query)
          ) {
        ...
        console.log(
          `📊 [${executionId}] Re-executing search with parameters: industry=${industry}, location=${location}, company=${companyName}`
        );
        ...
        if (req.body.businesses && Array.isArray(req.body.businesses)) {
          ...
          return res.status(400).json({
            error: "Missing data",
            message:
              "Please provide either a searchId or businesses data to export",
          });
        }
      }
      ...
      if (sheetName === "Lead Data" && searchParams.companyName)
        sheetName = searchParams.companyName;
      sheetName += ` - ${new Date().toLocaleDateString()}`;

      // Export the data
      console.log(
        `📊 [${executionId}] Exporting ${businesses.length} businesses to Google Sheets (ID: ${spreadsheetId})`
      );
      ...
      return res.status(200).json({
        ...
        data_exported: {
          ...
          contacts: businesses.reduce(
            (count, b) => count + (b.contacts?.length || 0),
            0
          ),
        },
      });
    ...
    try {
      ...
      if (!isAuthorized) {
        return res.status(200).json({
          ...
          setup_instructions: {
            message:
              "Google Sheets API is not configured. You need to provide either a service account JSON or an API key.",
            options: [
              {
                name: "Service Account (Recommended)",
                env_var: "GOOGLE_APPLICATION_CREDENTIALS_JSON",
                description:
                  "Full service account JSON credentials from Google Cloud Console",
              },
              {
                name: "API Key",
                env_var: "GOOGLE_API_KEY",
                description:
                  "Simple API key from Google Cloud Console (limited functionality)",
              },
            ],
            additional_settings: [
              {
                ...
                description:
                  "ID of an existing Google Sheet to export data to (optional)",
              },
              {
                name: "Auto-Export",
                env_var: "AUTO_EXPORT_TO_SHEETS",
                description:
                  "Set to 'true' to automatically export search results to Google Sheets",
              },
            ],
          },
        });
  ...
  app.get("/api/places-status", async (req: Request, res: Response) => {
    try {
      ...
      const { googlePlacesService } = await import(
        "./api/google-places-service"
      );
    ...
    try {
      ...
      if (!googleSheetsService.isAuthorized()) {
        return res.status(400).json({
          ...
          message:
            "Google Sheets API is not properly configured. Check /api/sheets-status for setup instructions.",
        });
      }
  ...
  app.get("/scrape/self-test", async (req: Request, res: Response) => {
    try {
      ...
      return res.status(200).json({
        status:
          testReport.passed_tests === testReport.total_tests
            ? "success"
            : "partial_success",
        timestamp: new Date().toISOString(),
  ...
  app.get("/scrape", async (req: Request, res: Response) => {
    try {
      ...
      const executionId = `scrape-${Date.now()}-${Math.floor(
        Math.random() * 1000
      )}`;

      console.log(
        `🔍 [${executionId}] API /scrape route called with query parameters:`,
        req.query
      );
      ...
      if (!query && !location) {
        console.log(
          `❌ [${executionId}] Missing required query parameters (query or location)`
        );
        return res.status(400).json({
          error: "Missing parameters",
          message:
            "Please provide at least one of: 'query' (company or industry) or 'location' parameter",
          timestamp: new Date().toISOString(),
          execution_id: executionId,
        });
      }
      ...
      console.log(
        `🔍 [${executionId}] Processing scrape request for query: '${query}', location: '${location}', page: ${page}, limit: ${limit}`
      );
      ...
      console.log(
        `🔍 [${executionId}] Executing real business search with Google Places API`
      );
      ...
      (scrapingResult.meta as any).total_contacts_generated =
        result.businesses.reduce(
          (count, b) => count + (b.contacts?.length || 0),
          0
        );
      (scrapingResult.meta as any).businesses_with_contacts =
        result.businesses.filter((b) => (b.contacts?.length || 0) > 0).length;
      (scrapingResult.meta as any).businesses_without_contacts =
        result.businesses.filter((b) => (b.contacts?.length || 0) === 0).length;
      (scrapingResult.meta as any).businesses_with_phone =
        result.businesses.filter(
          (b) => b.phoneNumber && b.phoneNumber.length > 0
        ).length;
      (scrapingResult.meta as any).businesses_with_website =
        result.businesses.filter(
          (b) => b.website && b.website.length > 0
        ).length;
      (scrapingResult.meta as any).data_quality_score = dataQualityScore;
      (scrapingResult.meta as any).api_quota_status =
        result.meta?.quotaStatus || "unknown";
      ...
      console.log(
        `📊 [${executionId}] Search metrics: ${result.businesses.length} businesses found`
      );
      if (result.meta) {
        console.log(
          `📊 [${executionId}] API metrics: ${result.meta.totalProcessedBusinesses}/${result.meta.totalResultsFound} businesses processed from ${result.meta.pagesRetrieved} pages`
        );
        console.log(
          `📊 [${executionId}] Generated ${result.meta.totalContactsGenerated} total contacts`
        );
        console.log(
          `📊 [${executionId}] Data quality score: ${dataQualityScore}/100`
        );

        // Log validation metrics
        const businessesWithNoContacts =
          (scrapingResult.meta as any).businesses_without_contacts || 0;
        if (businessesWithNoContacts > 0) {
          console.log(
            `⚠️ [${executionId}] ${businessesWithNoContacts} businesses had no extractable contacts`
          );
        }

        const businessesWithoutContactMethods = result.businesses.filter(
          (b) =>
            (!b.phoneNumber || b.phoneNumber.length === 0) &&
            (!b.website || b.website.length === 0)
        ).length;
        if (businessesWithoutContactMethods > 0) {
          console.log(
            `⚠️ [${executionId}] ${businessesWithoutContactMethods} businesses had no contact methods (phone/website)`
          );
        }

        // Log quota status
        console.log(
          `📊 [${executionId}] API quota status: ${
            (result.meta as any).quotaStatus || "unknown"
          }`
        );
      }
      ...
      if (
        ...
        scrapingResult.businesses.length === 0
      ) {
        console.log(
          `❌ [${executionId}] No real business data found after scraping all sources in ${executionTime}ms`
        );
        ...
        return res.status(404).json({
          ...
          message:
            "No real business information could be found after searching multiple authentic sources including Google Maps, Yelp, and industry directories.",
          query,
          location,
          timestamp: new Date().toISOString(),
          execution_id: executionId,
      ...
      console.log(
        `✅ [${executionId}] Successfully scraped real business data: ${scrapingResult.businesses.length} results in ${executionTime}ms`
      );
      ...
      return res.status(200).json({
        ...
        data: scrapingResult.businesses.map((business: BusinessData) => ({
          ...
          contacts:
            business.contacts?.map((contact: Contact) => ({
              name: contact.name,
      ...
      if (
        process.env.AUTO_EXPORT_TO_SHEETS === "true" &&
        process.env.GOOGLE_SHEETS_ID
      ) {
        try {
          console.log(
            `📊 [${executionId}] Auto-exporting results to Google Sheets`
          );
          ...
          console.log(
            `📊 [${executionId}] Google Sheets export result:`,
            exportResult
          );
        } catch (exportError) {
          console.error(
            `❌ [${executionId}] Error exporting to Google Sheets:`,
            exportError
          );
      ...
      console.error(
        `❌ Error during scraping operation at ${errorTimestamp}:`,
        error
      );

      // Log detailed error information for debugging
      console.error(
        `Error stack: ${(error as any).stack || "No stack trace available"}`
      );
...
function calculateRelevanceScore(
  ...
  prioritizeDecisionMakers: boolean = true
): number {
  let score = 0;
...
async function fetchRealBusinessData(
  query: string,
  location?: string
): Promise<any> {
  console.log(
    `Starting real business data search for: ${query}, location: ${location}`
  );
  ...
  if (result.error) {
    console.log(
      `API error encountered: ${result.error.code} - ${result.error.message}`
    );
  ...
  if (result.businesses.length === 0) {
    console.log(
      `No real business data found for query: ${query}, location: ${location}`
    );
    return null;
  }

  console.log(
    `Found ${result.businesses.length} businesses for query: ${query}`
  );
...
function generateIndustrySpecificData(
  category: string,
  companyName: string,
  location?: string
): any {
  switch (category) {
    ...
    case "technology":
      return {
        ...
        contacts: generateIndustryContacts(
          3,
...
function generateMoreContacts(
  ...
  switch (category) {
    ...
    case "retail_manufacturing":
      return generateIndustryContacts(
        count,
        companyName,
...
function generateBusinessCompanyData(
  companyName: string,
  location?: string
): any {
  ...
  return {
    name: companyName,
    industry:
      businessIndustries[Math.floor(Math.random() * businessIndustries.length)],
    subIndustry:
      businessIndustries[Math.floor(Math.random() * businessIndustries.length)],
    specialties: businessSpecialties.slice(
      0,
    ...
    contacts: generateBusinessContacts(
...
function generateRandomContacts(count: number, companyName: string): any[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Generate phone
    const areaCode = ["415", "212", "512", "312", "206", "617"][
      Math.floor(Math.random() * 6)
    ];
    const phone = `(${areaCode}) 555-${Math.floor(
    ...
    const homeAddress = isDecisionMaker
      ? `${Math.floor(Math.random() * 9000) + 1000} ${
          ["Maple", "Oak", "Pine", "Cedar", "Elm", "Willow"][
            Math.floor(Math.random() * 6)
          ]
        } ${
          ["St", "Ave", "Blvd", "Dr", "Ln", "Way"][
            Math.floor(Math.random() * 6)
          ]
        }, ${["Apt", "Unit", "Suite"][Math.floor(Math.random() * 3)]} ${
          Math.floor(Math.random() * 900) + 100
        }`
      : null;
    ...
    const previousCompany =
      Math.random() > 0.6
        ? {
            name: `${
              ["Alpha", "Beta", "Nova", "Apex", "Prime", "Elite"][
                Math.floor(Math.random() * 6)
              ]
            } ${
              [
            ...
            title:
              position.includes("C") || position.includes("VP")
    ...
    contacts.push({
      ...
      influence: isDecisionMaker
        ? Math.floor(Math.random() * 30) + 70
        : Math.floor(Math.random() * 50) + 20,
      budget: isDecisionMaker
      ...
      linkedIn:
        Math.random() > 0.3
          ? `linkedin.com/in/${firstName.toLowerCase()}-${lastName.toLowerCase()}-${Math.floor(
              Math.random() * 999
            )}`
          : null,
      twitter:
      ...
      notes: isDecisionMaker
        ? `${firstName} is a key decision-maker for vendor selection and has procurement authority.`
        : "",
    });
  }
...
function generateBusinessContacts(count: number, companyName: string): any[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i === 0 || Math.random() < 0.3) {
      ...
      position =
        businessPositions[Math.floor(Math.random() * businessPositions.length)];
    }
    ...
    const isDecisionMaker =
      ...
      position.includes("CFO") ||
      position.includes("CTO") ||
      position.includes("CIO") ||
      position.includes("CMO") ||
    ...
    contacts.push({
      ...
      meetings: isDecisionMaker ? Math.floor(Math.random() * 3) : 0,
      notes: isDecisionMaker
        ? `${firstName} is a key decision-maker for cleaning service vendor selection and has procurement authority.`
        : "",
    });
  }
