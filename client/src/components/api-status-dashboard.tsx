...
export default function ApiStatusDashboard() {
  const [placesApiStatus, setPlacesApiStatus] =
    useState<PlacesApiStatus | null>(null);
  ...
  const quotaColor =
    (placesApiStatus.quota_used_percent || 0) > 90
      ? "bg-red-500"
      : (placesApiStatus.quota_used_percent || 0) > 70
      ? "bg-yellow-500"
      : "bg-green-500";
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium">
                API Quota Usage (
                {(placesApiStatus.quota_used_percent || 0).toFixed(1)}%)
              </span>
              <span className="text-xs text-gray-500">
                {placesApiStatus.total_calls_24h || 0} /{" "}
                {placesApiStatus.quota_limit || 0} calls in 24h
              </span>
            </div>
            <Progress
              value={placesApiStatus.quota_used_percent || 0}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="text-2xl font-bold">
                {(placesApiStatus.total_calls_24h || 0) > 0
                  ? (
                      ((placesApiStatus.successful_calls_24h || 0) /
                        (placesApiStatus.total_calls_24h || 1)) *
                      100
                    ).toFixed(1)
                  : 0}
                %
          ...
          {(placesApiStatus.quota_used_percent || 0) > 80 && (
            <Alert className="mt-4 bg-yellow-50 border-yellow-200 text-yellow-800">
              <AlertCircle className="h-4 w-4" />
              <AlertTitle>High API Usage</AlertTitle>
              <AlertDescription>
                You've used{" "}
                {(placesApiStatus.quota_used_percent || 0).toFixed(1)}% of your
                Google Places API quota. Consider limiting searches until the
                quota resets.
              </AlertDescription>
            </Alert>
          )}
