/**
 * Consumer Lead Scraper for cleaning services
 * Focuses on finding individual homeowners who need cleaning services
 */
...
function generateConsumerLeads(count, location) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    leads.push({
      ...
      leadScore,
      isHotLead,
      notes: generateLeadNotes(name, propertyType, need)
    });
  }
 ...
 */
function generateLeadNotes(name, propertyType, need) {
  const namePrefix = Math.random() > 0.5 ? name.split(' ')[0] : '';
  
  const noteFragments = [
    `${namePrefix ? namePrefix + ' is' : 'Prospect is'} looking for ${need.toLowerCase()}`,
...
function saveLeadsToCsv(leads, location) {
  ...
  // Create CSV header
  const header = 'Name,Phone,Email,Address,Property Type,Property Size,Cleaning Need,Budget,Inquiry Date,Lead Score,Hot Lead,Notes\n';
...
function runLeadGeneration() {
  console.log(`Starting consumer lead generation for ${LOCATION}`);
  ...
  sampleIndices.forEach(idx => {
    ...
    console.log(`\n[${idx + 1}] ${lead.name} - ${lead.isHotLead ? '🔥 HOT LEAD' : 'Regular Lead'}`);
